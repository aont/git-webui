...
2. Optionally specify a target branch. If the branch does not exist it will be created. You can also switch modes to create a new branch from a commit ID, start an orphan branch, reset an existing branch to a specific commit and force-push it, or create a tag on a specific commit and push it, or delete a specified tag from the remote.
...
- `repository_url`
- `branch`
- `branch_mode` (`default`, `from_commit`, `orphan`, `revert_to_commit`, `tag_commit`, or `delete_remote_tag`)
- `new_branch`
- `base_commit`
- `tag_name`
- `git_user`
